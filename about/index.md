...
除此之外，我也具备一定的工程开发能力，能够

1. 为团队开发工具和系统，或部署一些开源的工具和系统。将重复出现的工作自动化完成，提高数据分析，建模，报告的效率
2. 参与生产系统的开发，将数据分析和建模的成果投入实际的线上产品。
...
* 解决方案：
  1. 我清洗和加工了多个和放贷相关的数据源的数据，用于分析放贷申请特征数据和坏账之间的关系。内容上包括支付业务数据，小贷数据，第三方同盾数据，通话详单数据等。存储形式上包含Oracle, Mongodb,  web API, excel文件等。
